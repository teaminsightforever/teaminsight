...
###### CONFIGURATION - see UCP.cfg  ##################
## Ensure Perl is installed on Slave servers ##
#my $servers = GetSetting('servers','UCP.cfg');

my $servers = "lon5dpu109:20140712>20140712,"; # Specify the servers and the dates they should run. syntax server1:staredate>enddate,server2:staredate>enddate,
...
my $cleanServersAfter = "false"; #set to true if you want the script to delete the files from the slave machines afterwards.
my $createMasterLookupFile = "true"; #set to true if you want the script to create a master lookup file after its finished.
my $lookupDays = 395; #set the number of days to go back when creating the combined (master) lookup file
