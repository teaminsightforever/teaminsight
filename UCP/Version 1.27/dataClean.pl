...
my $columnsPassedString = GetSetting('columnsPassed',$mainLoc);
...
my @columnsPassed = split(",",$columnsPassedString);
...
if($serverType eq "Slave")
{
	...
	#fine the index of column 203 as this is used in the filtering of the data. We need to do this as we are only extracting the relevant columns from the file, not them all (this is faster than reading in all of the columns into an array - split)
	my $want = 203;
	my $index = 0;
	++$index until $columnsPassed[$index] == $want or $index > $#columnsPassed;
	addToLog("$index should be the index location of field 203 in the columnsPassed parameter");
	...
	foreach(@nasFiles)
	{
		...
		addToLog("Debug:$columns total cleaned columns will be outputted to DC file");
		...
		if ($cleanData eq "false")
		{
			...
			while (<INFILE>) {
				#version 2 of the clean;
				my @f = (split /\t/)[@columnsPassed];
				if($f[0] == 1 || ($f[0] == 2 && $f[$index])){
					my $line = join("\t", @f);
					print DC_OUTFILE "$line\t\n";
				}
				
				#version 1 of the clean (slower)
				#my @f = (split /\t/);
